...
use crate::agent::communication::protocol::{AgentMessage, AgentMessageType};
...
pub struct WorkflowEngine {
    ...
    /// When `true`, steps are dispatched via the team's `AgentMessageBus`
    /// instead of calling `process_direct` directly.
    event_driven: bool,
}
...
impl WorkflowEngine {
    /// Create an engine with no persistence (in-memory only), direct-call mode.
    pub fn new() -> Self {
        Self {
    ...
    /// Enable event-driven execution mode.
    ///
    /// In this mode `WorkflowEngine` publishes `WorkflowStepAssigned` events
    /// to the team's bus and waits for `StepCompleted` replies instead of
    /// calling `process_direct` directly.  `AgentEventLoop` instances must be
    /// running on the team's bus for steps to complete.
    pub fn with_event_driven(mut self) -> Self {
        self.event_driven = true;
        self
    }

    /// Returns `true` if the engine is operating in event-driven mode.
    ...
    pub async fn execute_workflow(
        ...
        for layer_indices in &layers {
            ...
            if layer_indices.len() == 1 {
                ...
                let result = if self.event_driven {
                    run_step_with_retries_event_driven(
                        step.clone(),
                        team.clone(),
                ...
                let futs: Vec<_> = if self.event_driven {
                    layer_indices
                        .iter()
                        .map(|&idx| {
                            ...
                            futures_util::future::Either::Left(async move {
                                ...
                                run_step_with_retries_event_driven(step, team, ctx, wid, bus, 3)
                                    .await
                            })
                        })
                        .collect()
                    ...
                    layer_indices
                        ...
                        .map(|&idx| {
                            ...
                            futures_util::future::Either::Right(async move {
                                run_step_with_retries(step, team, ctx, 3).await
                            })
                        })
                        .collect()
                };
...
// ---------------------------------------------------------------------------
// Step execution — event-driven mode
// ---------------------------------------------------------------------------

/// Event-driven retry wrapper: mirrors `run_step_with_retries` but dispatches
/// via the `AgentMessageBus` instead of calling `process_direct` directly.
async fn run_step_with_retries_event_driven(
    step: WorkflowStep,
    team: Arc<AgentTeam>,
    context: HashMap<String, serde_json::Value>,
    workflow_id: String,
    ...
    for attempt in 0..max_retries {
        match run_workflow_step_event_driven(
            step.clone(),
            team.clone(),
            context.clone(),
            workflow_id.clone(),
        ...
        .await
        {
            ...
            Ok(_) => {
                warn!(
                    "Step '{}' failed (event-driven), retrying ({}/{})",
                    step.id,
                    attempt + 1,
                    max_retries
                );
            ...
            Err(e) => {
                warn!(
                    "Step '{}' error (event-driven): {}, retrying ({}/{})",
                    step.id,
                    e,
                    attempt + 1,
                    max_retries
...
/// Publish `WorkflowStepAssigned` to the bus and wait for the matching
/// `StepCompleted` reply.  Falls back to a timeout from `step.timeout`
/// (default 5 minutes).
async fn run_workflow_step_event_driven(
    step: WorkflowStep,
    team: Arc<AgentTeam>,
    context: HashMap<String, serde_json::Value>,
    workflow_id: String,
    ...
    // Resolve the agent that should handle this step
    let members = team.get_members_by_role(&step.role);
    let member = members.into_iter().next().ok_or_else(|| {
        crate::error::MofaclawError::Other(format!(
            "No agent with role '{}' in team '{}'",
    ...
    let engine_id = AgentId::new(&team.id, "workflow", "engine");

    // Subscribe BEFORE publishing to avoid a race where the agent replies
    // before the engine has a receiver open.
    let timeout = step.timeout.unwrap_or(Duration::from_secs(300));
    let start_time = std::time::Instant::now();
    let mut receiver = bus.subscribe_agent(&engine_id);
    ...
    bus.publish(msg).await.map_err(|e| {
        crate::error::MofaclawError::Other(format!(
            "Failed to publish WorkflowStepAssigned for step '{}': {}",
            step.id, e
        ))
    })?;

    info!(
        "Published WorkflowStepAssigned for step '{}' to agent '{}'",
        step.id, member.agent_id
    );

    loop {
        if start_time.elapsed() > timeout {
            warn!(
                "Timeout waiting for StepCompleted for step '{}' (event-driven)",
                step.id
            );
            return Ok(StepResult {
                step_id: step.id.clone(),
                ...
                artifacts: vec![],
                completed_at: Utc::now(),
                error: Some(format!("Timeout waiting for step '{}' completion", step.id)),
            });
        }
        ...
        match tokio::time::timeout(remaining, receiver.recv()).await {
            Ok(Ok(msg)) => {
                ...
                {
                    if wid == &workflow_id && sid == &step.id {
                        info!(
                            "Received StepCompleted for step '{}' (success={})",
                            step.id, success
                        );
                        return Ok(StepResult {
                            step_id: step.id.clone(),
            ...
            Ok(Err(_)) => {
                warn!(
                    "Bus closed while waiting for StepCompleted for step '{}'",
                    step.id
                );
                return Ok(StepResult {
                    step_id: step.id.clone(),
                    ...
                    artifacts: vec![],
                    completed_at: Utc::now(),
                    error: Some("Message bus closed unexpectedly".to_string()),
                });
            }
            Err(_) => {
                warn!(
