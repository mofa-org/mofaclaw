...
use crate::agent::communication::{AgentId, AgentMessage};
...
pub struct SharedWorkspace {
    ...
    /// TeamManager used to look up all active team buses when publishing
    /// artifact-change events.  Stored as a `Weak` to avoid a reference cycle.
    team_manager: Option<Weak<TeamManager>>,
}
...
impl SharedWorkspace {
    ...
    /// Attach a `TeamManager` so that artifact create/update operations
    /// broadcast `ArtifactUpdate` events to every active team's bus.
    /// A `Weak` reference is used to avoid a reference cycle.
    pub fn with_team_manager(mut self, manager: Weak<TeamManager>) -> Self {
        self.team_manager = Some(manager);
        self
    }

    /// Publish `msg` to every active team's `AgentMessageBus`.
    async fn publish_artifact_event(&self, msg: AgentMessage) {
        let Some(ref weak) = self.team_manager else {
            return;
        };
        let Some(manager) = weak.upgrade() else {
            return;
        };
        for bus in manager.all_team_buses().await {
            if let Err(e) = bus.publish(msg.clone()).await {
                warn!("Failed to publish ArtifactUpdate to team bus: {}", e);
            }
        }
    }
    ...
    pub async fn load(team_id: impl Into<String>, base_dir: PathBuf) -> Result<Self> {
        ...
        let workspace = Self::new(team_id.clone(), base_dir);
    ...
    pub async fn create_artifact_with_rbac(
        ...
        // Publish ArtifactUpdate event so event-loop agents can react
        let publisher = AgentId::new(&self.team_id, "workspace", "shared");
        let msg = AgentMessage::artifact_update(
            publisher,
            artifact.id.clone(),
            artifact.version,
            format!("Artifact '{}' created", artifact.name),
        );
        self.publish_artifact_event(msg).await;
    ...
    pub async fn update_artifact_with_rbac(
        ...
        let msg = AgentMessage::artifact_update(
            ...
            format!(
                "Artifact '{}' updated to v{}",
                new_artifact.name, new_artifact.version
            ),
        );
        self.publish_artifact_event(msg).await;
